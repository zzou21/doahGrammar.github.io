...
# Note to developer: do not forget to add work batch during interface design, so that the system automatically updates the correct dictionary routinely because the user cannot check all errors in one sitting. Could update the correct version dictionary after checking each historian's overview.
    ...
    def sentenceGrammarCheck(self):
        ...
        for historian, IdOverviewSegmentedList in self.DoAHSentSegDict.items():
            for Id, segmentedOverview in IdOverviewSegmentedList:
                for sentence in segmentedOverview:
...
if __name__ == "__main__":
    ...
    grammarCheckSentenceLevelMachine = grammarCheckSentenceLevel(doAHContentJsonFile, nameDictionary, doAHContentSentSegmentedJsonFile)
